...
- Базовый `docker-compose.yml` также привязывает приложение к `127.0.0.1:3000`, чтобы наружу торчал только Nginx.
- На сервере всегда используй связку `docker-compose.yml` + `docker-compose.prod.yml`; в итоговой конфигурации сервисы должны слушать только `127.0.0.1:3000` и `127.0.0.1:5432`, а не `0.0.0.0`.
